...
import { handleWelcome } from './welcome-modal.js';
...
playerCounter.addEventListener('change', () => {
  ...
  const playerCount = parseInt(playerCounter.value);
...
submitButton.addEventListener('click', e => {
  ...
  const nameArr = filterNames(getPlayerData(nameInputFields));
...
const addInputs = (count) => {
  ...
  for each player (according to counter selection), 
    ...
    2) join the array elems and write it to the inner html of the 'player-details-container' div 
  */
  const output = [];
...
const addPlayer = (data) => {
  data.forEach(d => {
    players.push({
      ...
      gotRaces: [],
      displayData(c) {
        const choiceText = c > 1 ? ' Choose One: <br>' : ' Plays as ';
        let showRaces = '';
      ...
      getRace(races) {
        ...
        const racePick = races.splice(0, 1);
...
(function createGreeting() {
  ...
  const greetMessage = /*html*/ `
    ...
    <div class="greeting-text" id="greet-text1" >
    ...
    <div class="welcome-footer">(click me to proceed)</div>
  `;
